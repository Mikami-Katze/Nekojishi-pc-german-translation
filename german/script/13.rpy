# TODO: Translation updated at 2018-02-23 08:51

# game/script/13.rpy:9
translate german s13_87da3a9f:

    # b "Every Sunday our club holds self practice. "
    b ""

# game/script/13.rpy:11
translate german s13_ac9a5b21:

    # b "Not many people turned up this week due to mid-terms. At least it's really a great way to relax for the people who finished early. Which is why I decided to come today. "
    b ""

# game/script/13.rpy:13
translate german s13_04f307f0:

    # b "However, I didn't expect to see Senpai here. After yesterday things were a bit awkward but we somewhat manage to say hi. "
    b ""

# game/script/13.rpy:15
translate german s13_c9cfbd03:

    # b "The club leader starts to go over the dance we're doing for our next show. "
    # b "I do my best to learn it but I doubt I'll end up performing on stage. "
    b ""

# game/script/13.rpy:19
translate german s13_1c1d4e55:

    # lpd normal "\"......\" "
    lpd normal ""

# game/script/13.rpy:23
translate german s13_6f921960:

    # b "Leopard's with me, and he absolutely puts his everything into the routine. He follows on flawlessly. "
    # b "He'd be great on stage... even if he'd scare some people at first. "
    b ""

# game/script/13.rpy:25
translate german s13_883fc54f:

    # clubleader "\"Alright, everyone. I want to see you guys try it by yourselves. Just let me know if you need a hand or ask one of our senior students up the front. They've done the routine before.\" "
    clubleader ""

# game/script/13.rpy:33
translate german s13_047e2060:

    # b "Senpai's one of the guys standing at the front... When did he get the chance to learn today's routine? He's been with me the whole time before exam break. There hasn't been any club meetings since... "
    b ""

# game/script/13.rpy:35
translate german s13_762c8900:

    # lpdcat normal "\"Liao, you alright? You're kinda spacing out over there. If you're stuck I'm happy to go through it with you.\" "
    lpdcat normal ""

# game/script/13.rpy:37
translate german s13_54f321e3:

    # b "Senpai begins to get in position for the first move.  "
    b ""

# game/script/13.rpy:39
translate german s13_761bad65:

    # avt "\"It's fine, I was just thinking to myself.\" "
    avt ""

# game/script/13.rpy:43
translate german s13_c3dfb710:

    # lpdcat laugh "\"Hmm, well while you're here you should take it as a chance to relax. Alright? Ready! Let's start!\" "
    lpdcat laugh ""

# game/script/13.rpy:49
translate german s13_8f1df927:

    # b "Senpai ends up forcing me to practice with him. "
    b ""

# game/script/13.rpy:54
translate german s13_3dcfe980:

    # b "Leopard comes over to dance with us too. Their moves flow in sync while mine pale in comparison... I know Senpai's been practicing for years but how did Leopard get so good? "
    b ""

# game/script/13.rpy:56
translate german s13_1b91d18d:

    # b "I can hardly keep up. I'm frustrated with myself but I know I can't really be expected to manage this on my first few times through, it's just too advanced. "
    # b "The best I can do is kinda jump around, vaguely following along as I piece everything together. "
    b ""

# game/script/13.rpy:61
translate german s13_182971ad:

    # lpdcat normal "\"Umm— That's not right. It's like this, see?\" "
    lpdcat normal ""

# game/script/13.rpy:66
translate german s13_9b8a4aaa:

    # lpd normal "\"Liao, look at me...\" "
    lpd normal ""

# game/script/13.rpy:70
translate german s13_67ae15dd:

    # b "Why is Leopard teaching me now! "
    b ""

# game/script/13.rpy:72
translate german s13_cddfc6eb:

    # b "My self-confidence falls apart entirely as they tear my dancing to shreds... {w=0.5}\n(I mean it wouldn't be the first time...) "
    b ""

# game/script/13.rpy:76
translate german s13_07c3b7e6:

    # b "At least some stuff get's easier after they break them down. But even knowing how they're performed the hardest parts are still way beyond my skills. "
    b ""

# game/script/13.rpy:80
translate german s13_12eb72f4:

    # b "One part's basically just showing off and it's not like I can do anything amazing. After all, this is meant to be for the end of year performance, it's not like it's going to be easy... "
    b ""

# game/script/13.rpy:82
translate german s13_1c1c7fae:

    # b "I don't have Senpai's agility let alone be dance in the face of the laws of physics like Leopard. I try, and I try. In the end I just decide to give up. "
    b ""

# game/script/13.rpy:87
translate german s13_c8c5ae48:

    # avt "\"It doesn't work, it just doesn't work. I can't do it.\" " with vshake
    avt "" with vshake

# game/script/13.rpy:89
translate german s13_c1fd89fd:

    # lpdcat normal "\"Alright, that's enough. Just take a break.\" "
    lpdcat normal ""

# game/script/13.rpy:96
translate german s13_ae08fb14:

    # lpd normal "\"...Liao. Again.\" "
    lpd normal ""

# game/script/13.rpy:98
translate german s13_c90c360d:

    # b "For some reason Leopard really wants me to try agian. "
    b ""

# game/script/13.rpy:100
translate german s13_c044341f:

    # avt "\"......?\" "
    avt ""

# game/script/13.rpy:102
translate german s13_c6829df2:

    # b "I don't get it, but he really does earnestly want me to try. So I decide to give it one more go for him. "
    b ""

# game/script/13.rpy:106
translate german s13_cbe00327:

    # b "I get up to a hard part, I know I can't do it but I might as well anyway. "
    # b "Huh? Wait that worked? "
    b ""

# game/script/13.rpy:109
translate german s13_776bf21f:

    # lpdcat surprise_1 "\"What...\" "
    lpdcat surprise_1 ""

# game/script/13.rpy:111
translate german s13_df21657f:

    # tig surprise "\"What? Hold on, you're...\" "
    tig surprise ""

# game/script/13.rpy:121
translate german s13_93f87bfa:

    # b "My body feels light as if my weight had left my limbs. I'm even able to pull off a handstand spin perfectly without even having to worry about balancing my momentum. "
    b ""

# game/script/13.rpy:136
translate german s13_db30bdb1:

    # b "Moving in to the second half I breeze through the hardest part of the routine as if it didn't faze me at all! "
    # b "But this is my body! Before I was just surprised by what happened but it's all just impossible. " with vshake
    b "" with vshake

# game/script/13.rpy:138
translate german s13_50492cac:

    # b "There's no way I can do this... But how is this..."
    b ""

# game/script/13.rpy:147
translate german s13_cc0c41f6:

    # b "Before I work out what's going on I manage to finish routine. As I hold the final pose I hear a wave of applause shower me from everyone in the room. " with hshake
    b "" with hshake

# game/script/13.rpy:151
translate german s13_b819b232:

    # clubmate "\"Wow! Liao, you did great!\" " with vshake
    clubmate "" with vshake

# game/script/13.rpy:154
translate german s13_9b55b954:

    # clubmate "\"How did you learn it so fast?\" " with vshake
    clubmate "" with vshake

# game/script/13.rpy:157
translate german s13_1606f380:

    # clubmate "\"You must have been secretly practicing this whole time! \nI knew Shu-Chi Senpai was—\" " with vshake
    clubmate "" with vshake

# game/script/13.rpy:161
translate german s13_4572314a:

    # b "The last guy was quickly pulled to the side by someone else. " with flash
    b "" with flash

# game/script/13.rpy:163
translate german s13_9a59531b:

    # b "I stand there, confused and out of breath. I {i}know{/i} \ I couldn't possibly have pulled that off. "
    # b "There's no way I could have just learnt all of it just like that. What's going on... "
    b ""

# game/script/13.rpy:171
translate german s13_f3556ac8:

    # b "Senpai looks on with a face of embarrassment. Tiger on the other hand is furious. "
    b ""

# game/script/13.rpy:173
translate german s13_19494e1e:

    # avt "\"What happened...\" "
    avt ""

# game/script/13.rpy:181
translate german s13_fdc0e027:

    # lpdcat panic "\"Oh... Liao, you danced so well! Come here.\" "
    lpdcat panic ""

# game/script/13.rpy:184
translate german s13_91d443af:

    # b "Someone in the crowd wolf wistles at us, but I follow Senpai over to a corner anyway. "
    b ""

# game/script/13.rpy:187
translate german s13_968d4dc7:

    # lpdcat difficult "\"Err...{w=0.5} Kulau, You've crossed the line...\" "
    lpdcat difficult ""

# game/script/13.rpy:191
translate german s13_1553364e:

    # lpd obsessed "\"......?\" "
    lpd obsessed ""

# game/script/13.rpy:199
translate german s13_26671b89:

    # tig angry "\"What did you do? Did I not tell you to not touch Liao? Or what? Did you think it didn't matter as long as you didn't do something evil?\" "
    tig angry ""

# game/script/13.rpy:202
translate german s13_425845a4:

    # b "Wait, wait— What did he do to me? "
    b ""

# game/script/13.rpy:204
translate german s13_47d0c662:

    # lpdcat difficult "\"Liao, Kulau just took control of your body. Didn't you notice?\" "
    lpdcat difficult ""

# game/script/13.rpy:206
translate german s13_7b6349e7:

    # avt "\"What...\" "
    avt ""

# game/script/13.rpy:208
translate german s13_91026b9d:

    # b "I know I couldn't have done that myself, but I was {i}controlled?{/i} \ I didn't notice \nanything... "
    # b "I guess it kinda felt like my body was moving by itself, but it also really did feel like I was in control. I didn't think about it that much... "
    b ""

# game/script/13.rpy:210
translate german s13_5424d150:

    # lpd obsessed "\"Can, not do?\" "
    lpd obsessed ""

# game/script/13.rpy:212
translate german s13_dfa1fee8:

    # lpdcat difficult "\"Well, a bit of help is fine but you didn't even let him do anything by himself. I don't really care but you probs pissed of the old man off...\" "
    lpdcat difficult ""

# game/script/13.rpy:215
translate german s13_ac07d927:

    # tig angry "\"Of course!\" " with vshake
    tig angry "" with vshake

# game/script/13.rpy:217
translate german s13_868c4780:

    # lpdcat difficult "\"It would have been better to help him out when he's back home though. What do you think is gonna happen now that everyone has seen him dance so well? What happens when he messes it up next time?\" "
    lpdcat difficult ""

# game/script/13.rpy:222
translate german s13_7119a2db:

    # tig angry "\"I've told you to think about the consequences of your actions!\" "
    tig angry ""

# game/script/13.rpy:226
translate german s13_8834b3a0:

    # tig angry "\"Did you not think about the other people around you? What is wrong with you?\" "
    # tig angry "\"Would you wish for Liao to never perform in front of the others again? Or would you rather dance for him for now on?\" "
    # tig angry "\"Maybe you'd rather attend every dance club meeting on Liao's behalf? Do you understand what you just did!\" "
    tig angry ""

# game/script/13.rpy:228
translate german s13_592ea892:

    # lpdcat serious "\"Drop it, old man. It doesn't really matter... I'll just tell them I've been teaching Liao...\" "
    lpdcat serious ""

# game/script/13.rpy:230
translate german s13_c4c4e511:

    # tig angry "\"Liao danced unreasonably well just then. How could you implore them to believe you after witnessing that?\" "
    tig angry ""

# game/script/13.rpy:232
translate german s13_40830035:

    # tig angry "\"Will they not look for another reason? What if they leap to some odd conclusion?\" "
    # tig angry "\"Even if they believe your story about teaching them, kitten. It would only serve to add to the rumours about your relationship!\" "
    tig angry ""

# game/script/13.rpy:237
translate german s13_15a4fa0e:

    # tig angry "\"You fool. If you were ever to think about the consequences... How can you call yourself a god worthy to lead anyone?\" "
    tig angry ""

# game/script/13.rpy:242
translate german s13_ec7ea448:

    # tig angry "\"Look at what you've done. Liao might have to quit his club over this! You've put Liao's dreams in jeopardy! You may even have ruined them.\" "
    tig angry ""

# game/script/13.rpy:246
translate german s13_3b5e46f7:

    # lpd obsessed "\"......\" "
    lpd obsessed ""

# game/script/13.rpy:248
translate german s13_7469205a:

    # lpd obsessed "\"Ruined... {w=0.5}Liao's dream?\" "
    lpd obsessed ""

# game/script/13.rpy:250
translate german s13_3b23b615:

    # avt "\"Umm...\" "
    avt ""

# game/script/13.rpy:254
translate german s13_0a96c187:

    # b "I don't think it's that serious. It's just gonna be— well, awkward. "
    b ""

# game/script/13.rpy:256
translate german s13_cfb830cd:

    # b "At the very least I won't be able to practice that routine during club time anymore, enough people saw me that it wouldn't make sense if I sucked at it next time. "
    b ""

# game/script/13.rpy:258
translate german s13_45ab529e:

    # b "But what if they ask me to be in the end of semester performance... I'll be forced to beg Leopard to help me. "
    b ""

# game/script/13.rpy:260
translate german s13_45d4daba:

    # b "They might just think I'm really good now and ask me to perform at every show... and I'll just be asked, over and over again... "
    b ""

# game/script/13.rpy:262
translate german s13_b37c02ce:

    # b "Dancing is about your ability to coordinate and move in rhythm, it's just gonna be really weird for me to struggle after doing all of that flawlessly. "
    # b "My head hurts just thinking about how to explain it to everyone... "
    b ""

# game/script/13.rpy:266
translate german s13_e311497b:

    # avt "\"I mean, it's not that bad. Worst case I just leave dance club. It's not like I couldn't just learn dancing anywhere...\" "
    avt ""

# game/script/13.rpy:268
translate german s13_f71b1b1a:

    # tig angry "\"Do you really expect to become proficient without a teacher let alone a venue? If you did leave, would you pay for those resources?\" "
    tig angry ""

# game/script/13.rpy:270
translate german s13_b1e13756:

    # avt "\"You're just exaggerating, I should be able to work past this...\" "
    avt ""

# game/script/13.rpy:272
translate german s13_662571cd:

    # tig frown "\"Sure, you might be able to get away this time. But what about next? And after that?\" "
    tig frown ""

# game/script/13.rpy:277
translate german s13_a0584ea8:

    # tig angry "\"Aborigine, I knew you were trouble during the math exam. Now you've just proved me right!\" "
    tig angry ""

# game/script/13.rpy:281
translate german s13_6cfb7540:

    # tig angry "\"If you cannot figure out how the modern world functions, you shouldn't get in the way of others!\" "
    tig angry ""

# game/script/13.rpy:283
translate german s13_0e9380de:

    # tig angry "\"Who knows what you'll interfere with next? You're just getting in the way of Liao's \nlife!\" "
    tig angry ""

# game/script/13.rpy:288
translate german s13_6a83f5e4:

    # lpdcat difficult "\"Hey— Old man. Calm down for a sec...\" "
    lpdcat difficult ""

# game/script/13.rpy:291
translate german s13_24940dd1:

    # tig angry "\"Leave me be.\" "
    tig angry ""

# game/script/13.rpy:293
translate german s13_3b5e46f7_1:

    # lpd obsessed "\"......\" "
    lpd obsessed ""

# game/script/13.rpy:295
translate german s13_f91f559e:

    # tig angry "\"I don't care what this kitten says.\" "
    tig angry ""

# game/script/13.rpy:298
translate german s13_11c06e42:

    # lpdcat not_reconcile "\"I'm not a kitten!\" "
    lpdcat not_reconcile ""

# game/script/13.rpy:300
translate german s13_f5de9dbe:

    # tig frown "\"I can't let this continue. If something like this were to happen again, who knows if it could be mended.\" "
    # tig frown "\"It's not worth the risk to allow you to stay.\" "
    tig frown ""

# game/script/13.rpy:302
translate german s13_6c037936:

    # tig frown "\"Now, I will only tell you one more time. Liao has no obligation to help you, and he has no obligation to have you by his side.\" "
    # tig frown "\"If you are to interfere with his life, you are better off keeping your distance. For Liao's sake.\" "
    tig frown ""

# game/script/13.rpy:304
translate german s13_6f51b0c9:

    # tig frown "\"Understood?\" "
    tig frown ""

# game/script/13.rpy:310
translate german s13_3b5e46f7_2:

    # lpd obsessed "\"......\" "
    lpd obsessed ""

# game/script/13.rpy:312
translate german s13_db0e0aeb:

    # avt "\"Tiger, don't be so fierce with him...\" "
    avt ""

# game/script/13.rpy:315
translate german s13_9266c07f:

    # tig angry "\"Liao! ...Don't not talk back to me.\" " with vshake
    tig angry "" with vshake

# game/script/13.rpy:317
translate german s13_1f0b2e77:

    # tig frown "\"If you heard me clearly, you should know what to do.\" "
    tig frown ""

# game/script/13.rpy:319
translate german s13_1dc2082d:

    # lpdcat panic "\"Hey... just drop it. The old man's just {i}angry{/i} \ right now. You should—\" "
    lpdcat panic ""

# game/script/13.rpy:321
translate german s13_f51d4b2d:

    # lpd disappointed "\"Understood.\" "
    lpd disappointed ""

# game/script/13.rpy:323
translate german s13_fdbee339:

    # avt "\"Leopard...\" "
    avt ""

# game/script/13.rpy:325
translate german s13_b0ea0f32:

    # lpd disappointed "\"I leave.\" "
    lpd disappointed ""

# game/script/13.rpy:331
translate german s13_49d5f346:

    # avt "\"Hmm? Hold on... you...\" "
    avt ""

# game/script/13.rpy:333
translate german s13_713baaf2:

    # b "Before I can stop him, Leopard's gone. He vanished. I couldn't work out which way he went even if I wanted. "
    b ""

# game/script/13.rpy:335
translate german s13_776bf21f_1:

    # lpdcat surprise_1 "\"Wha...\" "
    lpdcat surprise_1 ""

# game/script/13.rpy:337
translate german s13_195b58df:

    # tig frown "*humph* "
    tig frown ""

# game/script/13.rpy:339
translate german s13_ac854a32:

    # avt "\"Tiger... That wasn't necessary!\" "
    avt ""

# game/script/13.rpy:341
translate german s13_1b6c760f:

    # tig frown "\"Liao, I'm sorry. It was for your own sake.\" "
    tig frown ""

# game/script/13.rpy:343
translate german s13_9bf6b401:

    # tig frown "\"At least one problem has been solved. You no longer need to worry about going to Wutai.\" "
    tig frown ""

# game/script/13.rpy:345
translate german s13_e970b177:

    # avt "\"How could you... He was trying to help me!\" " with hshake
    avt "" with hshake

# game/script/13.rpy:350
translate german s13_624b43d7:

    # b "He helps me, I help him... Leopard's words suddenly come to my mind. Is that... why he wanted to help me today? All of this is because I {i}didn't{/i} \ stop him? "
    b ""

# game/script/13.rpy:352
translate german s13_9593e1d7:

    # b "If I... really had no intentions to go to Wutai, I should have stopped him. I didn't deserve his help if I was never going to repay him... "
    b ""

# game/script/13.rpy:360
translate german s13_3b50b7ac:

    # tig frown "\"I know he just wanted to help you, but he doesn't understand what he's doing. You're the one who gets hurt in the end. \" "
    # tig frown "\"It's better to do something before it's too late.\" "
    tig frown ""

# game/script/13.rpy:362
translate german s13_626637ad:

    # avt "\"So you just decided to get rid of him!\" "
    avt ""

# game/script/13.rpy:364
translate german s13_92f803fc:

    # avt "\"I...\" "
    avt ""

# game/script/13.rpy:369
translate german s13_e88eca98:

    # b "Suddenly, I'm hit by everything Leopard's done for me. "
    # b "He always helped me during dance practice... "
    # b "That time he brought me coffee when I was exhausted... "
    # b "He helped me cheat in class even though he knew it was wrong... "
    # b "He protected the tung blossoms so we could at least take one of them home with us... "
    # b "And... {w=0.5}he comforted me when I felt sad and alone. "
    b ""

# game/script/13.rpy:373
translate german s13_ad7ba116:

    # avt "\"I-{w=0.2}I won't even have a chance to say thanks...\" "
    avt ""

# game/script/13.rpy:375
translate german s13_b1b38033:

    # tig normal "\"...I'm sorry.\" "
    tig normal ""

# game/script/13.rpy:377
translate german s13_97881033:

    # tig normal "\"But Liao, this is—\" "
    tig normal ""

# game/script/13.rpy:379
translate german s13_7c25ee6d:

    # b "Tiger stopped his sentence there. Looking onwards at each other in silence... It said everything that needed to be said. "
    b ""

# game/script/13.rpy:385
translate german s13_f9b0421b:

    # lpdcat panic "\"Wow... this is awkward...\" " with hshake
    lpdcat panic "" with hshake

# game/script/13.rpy:389
translate german s13_7ee38493:

    # lpdcat normal "\"Alright, enough is enough. It's not like this has to be the end. Maybe Kulau will come back.\" "
    lpdcat normal ""

# game/script/13.rpy:391
translate german s13_ce4bf2f3:

    # lpdcat normal "\"The old man can clear things up, and Liao would have plenty of time to talk, right?\" "
    lpdcat normal ""

# game/script/13.rpy:393
translate german s13_8cf15f21:

    # lpdcat wronged "\"There's no need to... worry so much...\" "
    lpdcat wronged ""

# game/script/13.rpy:395
translate german s13_03eb0388:

    # tig frown "*tch* "
    tig frown ""

# game/script/13.rpy:397
translate german s13_3b0fe3d9:

    # avt "\"Maybe... but...\" "
    avt ""

# game/script/13.rpy:399
translate german s13_8a4fc3c6:

    # b "What if he {i}doesn't{/i} \ come back? "
    b ""

# game/script/13.rpy:401
translate german s13_f9fac311:

    # avt "\"...I-{w=0.2}I want to go home.\" "
    avt ""

# game/script/13.rpy:403
translate german s13_0cefc785:

    # lpdcat serious "\"That's, a good idea. Liao's probably not in the mood for practise anyway.\" "
    lpdcat serious ""

# game/script/13.rpy:405
translate german s13_1f473cae:

    # lpdcat serious "\"Which means... I should probably take this chance to tell you.\" "
    lpdcat serious ""

# game/script/13.rpy:407
translate german s13_81029d2c:

    # lpdcat serious "\"I've asked for the day off class tomorrow. I'll be leaving tonight.\" "
    lpdcat serious ""

# game/script/13.rpy:409
translate german s13_fe51e72b:

    # lpdcat serious "\"You know, to go have a big fight with my family. Fun times.\" "
    lpdcat serious ""

# game/script/13.rpy:411
translate german s13_be08e90b:

    # lpdcat wronged "\"Yan Shu-Chi's on bad terms... I really don't know how bad it's gonna get, but that's just how it is...\" "
    lpdcat wronged ""

# game/script/13.rpy:413
translate german s13_bc396a40:

    # lpdcat wronged "\"And I plan to take this as my chance to... {w=0.5}leave him for good.\" "
    lpdcat wronged ""

# game/script/13.rpy:423
translate german s13_02_not_lpdcat_last_date_5a14c30e:

    # avt "\"Wha— Why so suddenly?\" "
    avt ""

# game/script/13.rpy:425
translate german s13_02_not_lpdcat_last_date_8836e935:

    # lpdcat serious "\"It's not out of nowhere... I've seen this coming for days. It's just been getting, more intense.\" "
    # lpdcat serious "\"I have to face it sooner or later.\" "
    lpdcat serious ""

# game/script/13.rpy:427
translate german s13_02_not_lpdcat_last_date_c5061851:

    # lpdcat serious "\"That's why I need to go back for a few days. This is Yan-Shu-Chi's family affair. You can't get involved...\" "
    lpdcat serious ""

# game/script/13.rpy:429
translate german s13_02_not_lpdcat_last_date_f8a6debb:

    # avt "\"But, this is partially to do with me isn't it?\" "
    avt ""

# game/script/13.rpy:431
translate german s13_02_not_lpdcat_last_date_291ee00a:

    # lpdcat serious "\"Yeah it is, but you'll getting involved with them will just make things worse.\" "
    lpdcat serious ""

# game/script/13.rpy:433
translate german s13_02_not_lpdcat_last_date_0f94d2fb:

    # lpdcat normal "\"I'll manage by myself. I'll do it. You just won't be able to see me for a few days, that's all.\" "
    lpdcat normal ""

# game/script/13.rpy:435
translate german s13_02_not_lpdcat_last_date_0af8170c:

    # lpdcat normal "\"You should, just go home now.\" "
    lpdcat normal ""

# game/script/13.rpy:440
translate german s13_02_not_lpdcat_last_date_a8118553:

    # b "But, Senpai..."
    b ""

# game/script/13.rpy:442
translate german s13_02_not_lpdcat_last_date_96dede30:

    # b "My heart turns over itself inside me, a mess of feelings where I don't know what to do. "
    b ""

# game/script/13.rpy:444
translate german s13_02_not_lpdcat_last_date_b1206d09:

    # tig normal "\"...I think we should go now.\" "
    tig normal ""

# game/script/13.rpy:446
translate german s13_02_not_lpdcat_last_date_3bf0dc1b:

    # avt "\"Oh, umm... yeah...\" "
    avt ""

# game/script/13.rpy:454
translate german s13_02_lpdcat_last_date_99add9c3:

    # lpdcat serious "\"Well, as you know, I'm going back to Miaoli pretty soon... There's no point in me staying here any longer.\" "
    # lpdcat serious "\"And I don't plan to jeopardise Yan Shu-Chi's relationship with his family any longer...\" "
    lpdcat serious ""

# game/script/13.rpy:456
translate german s13_02_lpdcat_last_date_dfa37256:

    # lpdcat serious "\"So, I guess this is the last time we'll see each other.\" "
    lpdcat serious ""

# game/script/13.rpy:458
translate german s13_02_lpdcat_last_date_6feb25c9:

    # lpdcat normal "\"Haha— You know, I always wanted to say goodbye to you with a big smile on your \nface.\" "
    lpdcat normal ""

# game/script/13.rpy:460
translate german s13_02_lpdcat_last_date_c58b3097:

    # avt "\"Senpai...\" "
    avt ""

# game/script/13.rpy:462
translate german s13_02_lpdcat_last_date_4b2df9a8:

    # lpdcat laugh "\"Don't worry about it. I can't make our feelings change on the flip of a coin. It was just a silly wish of mine.\" "
    lpdcat laugh ""

# game/script/13.rpy:464
translate german s13_02_lpdcat_last_date_e0603899:

    # lpdcat normal "\"Who knows, I might get a chance to come visit you sometime. Even if I'm a yaoguai.\" "
    lpdcat normal ""

# game/script/13.rpy:467
translate german s13_02_lpdcat_last_date_2e075607:

    # lpdcat cute "\"Old man, please show some mercy~\" "
    lpdcat cute ""

# game/script/13.rpy:469
translate german s13_02_lpdcat_last_date_dda38b34:

    # tig normal "\"What, as long as you don't cause trouble it wouldn't be worth my time to harm you.\" " with vshake
    tig normal "" with vshake

# game/script/13.rpy:471
translate german s13_02_lpdcat_last_date_e281da9d:

    # tig normal "\"You are welcome to visit... as long as you don't drag Liao back to Miaoli of course.\" "
    tig normal ""

# game/script/13.rpy:473
translate german s13_02_lpdcat_last_date_5c39ea54:

    # lpdcat laugh "\"Hehe— Thanks old man. Was that that hard to say? I guess even if I turn back into a yaoguai, by Fudegong could still be revived after all.\" "
    lpdcat laugh ""

# game/script/13.rpy:475
translate german s13_02_lpdcat_last_date_b87169c0:

    # tig normal "\"We shall see.\" "
    tig normal ""

# game/script/13.rpy:477
translate german s13_02_lpdcat_last_date_de6d588d:

    # lpdcat laugh "\"Hehe— Yep, we will. Okay Liao, guess I'll see you when I see you.\" "
    lpdcat laugh ""

# game/script/13.rpy:480
translate german s13_02_lpdcat_last_date_ec85c52e:

    # b "Senpai throws my backpack to me. "
    b ""

# game/script/13.rpy:482
translate german s13_02_lpdcat_last_date_eaa606b0:

    # lpdcat normal "\"Go home, I'll be sure to cover things up for you here.\" "
    lpdcat normal ""

# game/script/13.rpy:484
translate german s13_02_lpdcat_last_date_8110132d:

    # lpdcat laugh "\"Goodbye Liao~\" "
    lpdcat laugh ""

# game/script/13.rpy:486
translate german s13_02_lpdcat_last_date_2dd7fb8d:

    # avt "\"...Yeah.\" "
    avt ""

# game/script/13.rpy:488
translate german s13_02_lpdcat_last_date_df43061c:

    # avt "\"Goodbye Senpai.\" "
    avt ""

# game/script/13.rpy:498
translate german s13_02_lpdcat_last_date_829571bb:

    # b "It's bittersweet, but we say our goodbyes and I head home with Tiger. "
    b ""

# game/script/13.rpy:513
translate german s13_03_9a41103c:

    # b "Even later that night, Leopard hadn't come home. "
    b ""

# game/script/13.rpy:516
translate german s13_03_0a0fb2ec:

    # b "It's late and I'm wide awake... I just don't feel like doing anything. "
    # b "So I just sit at my computer scrolling though websites, not really reading anything. Doing what essentially amounts to nothing. "
    b ""

# game/script/13.rpy:521
translate german s13_03_9b80c920:

    # tig nervous "\"...Sorry, Liao.\" "
    tig nervous ""

# game/script/13.rpy:523
translate german s13_03_417d4d33:

    # avt "\"Hmm? Why the sudden apology?\" "
    avt ""

# game/script/13.rpy:525
translate german s13_03_3660c513:

    # tig nervous "\"I was too... excited. There was no need for me to be that forceful with him.\" "
    tig nervous ""

# game/script/13.rpy:527
translate german s13_03_22150f85:

    # avt "\"...I never thought you would say that.\" "
    avt ""

# game/script/13.rpy:529
translate german s13_03_24fae009:

    # tig nervous "\"I just wanted to get rid of him. I saw my opportunity and took it...\" "
    tig nervous ""

# game/script/13.rpy:531
translate german s13_03_d241b2b3:

    # avt "\"So you... drove him away.\" "
    avt ""

# game/script/13.rpy:533
translate german s13_03_07d45713:

    # tig embarrassed "\"Liao, I'm sorry. I didn't mean to hurt your feelings.\" "
    tig embarrassed ""

# game/script/13.rpy:535
translate german s13_03_141d98bb:

    # avt "\"Do I look upset?\" "
    avt ""

# game/script/13.rpy:539
translate german s13_03_ea4a6bc6:

    # b "Because I {i}am{/i} \ upset. Leopard left without a word and Senpai's going now as well. How could I not care? "
    b ""

# game/script/13.rpy:541
translate german s13_03_5c427d39:

    # b "I don't blame Tiger for it. He's never been the one who wanted them to be here in the first place. He was just provoked... I should have been the one to step in there. "
    b ""

# game/script/13.rpy:543
translate german s13_03_67db4a2f:

    # b "And I guess... {w=0.5}It just feels like I've lost something important. "
    b ""

# game/script/13.rpy:545
translate german s13_03_ac2ed14b:

    # b "It's all just happened so suddenly. "
    b ""

# game/script/13.rpy:549
translate german s13_03_3be0ea0c:

    # tig nervous "\"...Liao, would you rather they came back?\" "
    tig nervous ""

# game/script/13.rpy:555
translate german s13_03_5b22f87f:

    # avt "\"Things were just happier when they were around. As dumb as that sounds.\" "
    # avt "\"But... maybe they were just being nice to me because they wanted me to help them, right?\" "
    avt ""

# game/script/13.rpy:557
translate german s13_03_8c7d36bc:

    # avt "\"Did I really have any intentions to help them... It'd just be wrong to feel good for what they did for me. I'd just be taking advantage of them.\" "
    avt ""

# game/script/13.rpy:559
translate german s13_03_b306ae92:

    # tig nervous "\"Is that really what's important? Did you not enjoy their company?\" "
    tig nervous ""

# game/script/13.rpy:561
translate german s13_03_35d25cf4:

    # avt "\"Well...{w=0.5} There's a lot of things to think about other than whether I just liked them around or not. I'm conflicted...\" "
    avt ""

# game/script/13.rpy:563
translate german s13_03_dba713ba:

    # tig nervous "\"Liao, if you wish, I can get them back. At least the aborigine.\" "
    tig nervous ""

# game/script/13.rpy:565
translate german s13_03_77e13ac4:

    # avt "\"No, don't. He'll come back if he wants to. Asking him doesn't feel right...\" "
    avt ""

# game/script/13.rpy:569
translate german s13_03_2166e7b1:

    # avt "\"......\" "
    avt ""

# game/script/13.rpy:571
translate german s13_03_889adbd4:

    # tig disappointed "\"......\" "
    tig disappointed ""

# game/script/13.rpy:573
translate german s13_03_43fc51c6:

    # tig disappointed "\"Liao, do you think I'm too forceful?\" "
    tig disappointed ""

# game/script/13.rpy:575
translate german s13_03_2f96d97e:

    # avt "\"It's fine. It's just a part of who you are.\" "
    avt ""

# game/script/13.rpy:577
translate german s13_03_adbf85cf:

    # tig normal "\"Do you see me as too controlling?\" "
    tig normal ""

# game/script/13.rpy:579
translate german s13_03_e93f030f:

    # avt "\"A little, but I know it's only because you care about me.\" "
    avt ""

# game/script/13.rpy:584
translate german s13_03_7d05ed51:

    # tig normal "\"If you were to choose, would you rather not see us anymore?\" "
    tig normal ""

# game/script/13.rpy:586
translate german s13_03_c457b023:

    # avt "\"...Don't talk like that.\" " with hshake
    avt "" with hshake

# game/script/13.rpy:588
translate german s13_03_ced3eddd:

    # tig normal "\"Would you wish to stay with me and only me?\" "
    tig normal ""

# game/script/13.rpy:590
translate german s13_03_2166e7b1_1:

    # avt "\"......\" "
    avt ""

# game/script/13.rpy:592
translate german s13_03_6a1020d6:

    # tig normal "\"I've given a lot of thought about it. As to how things would be if you were to go \nhome.\" "
    # tig normal "\"The other guardian tigers, they're very traditional. Honestly, you'll probably be bothered by them more than anything.\" "
    tig normal ""

# game/script/13.rpy:594
translate german s13_03_50dc1aa9:

    # tig nervous "\"If you were to not go back... You are still a son of the Lin family. Could you really distinguish yourself from them forever?\" "
    tig nervous ""

# game/script/13.rpy:596
translate german s13_03_2132b41a:

    # tig nervous "\"I realise that driving your family away may not be the best for you, but I cannot accept you leaving for Miaoli or Wutai.\" "
    tig nervous ""

# game/script/13.rpy:598
translate german s13_03_a9b7f55f:

    # tig nervous "\"When the aborigine... {w=0.5}controlled your body, I was furious.\" "
    tig nervous ""

# game/script/13.rpy:600
translate german s13_03_3e94c03c:

    # avt "\"Why?\" "
    avt ""

# game/script/13.rpy:604
translate german s13_03_5c835f03:

    # tig nervous "\"You are a Jitong! What he did was almost possession, I couldn't let that go by.\" " with vshake
    tig nervous "" with vshake

# game/script/13.rpy:606
translate german s13_03_86d8d694:

    # tig embarrassed "\"In truth... he was moving your body with spirit power, it's not the same. But my feeling still stands. After all, it is the method one would control someone with weak spiritual power, such as your brother.\" "
    tig embarrassed ""

# game/script/13.rpy:608
translate german s13_03_bd06bbbf:

    # tig nervous "\"Watching as he... {w=0.5}manipulated your body as if you were his. {w=0.5}I couldn't take it.\" "
    tig nervous ""

# game/script/13.rpy:610
translate german s13_03_8bf59c02:

    # tig embarrassed "\"Alas. I doubt you understand all of this, and I know he meant you no harm. It's just...\" "
    tig embarrassed ""

# game/script/13.rpy:612
translate german s13_03_637b33dc:

    # avt "\"Tiger...\" "
    avt ""

# game/script/13.rpy:614
translate german s13_03_0a28077a:

    # avt "\"I'm sorry.\" "
    avt ""

# game/script/13.rpy:621
translate german s13_03_4ceb0ccc:

    # tig nervous "\"Haha— Now you're the one apologetic one.\" "
    tig nervous ""

# game/script/13.rpy:623
translate german s13_03_f8489cbd:

    # avt "\"I... {w=0.5}I haven't been thinking about how you feel.\" "
    avt ""

# game/script/13.rpy:625
translate german s13_03_769c8ca5:

    # tig laugh_1 "\"No, it's fine. I'm a god. It's my duty to protect my worshippers. They shouldn't be worrying about our feelings.\" "
    tig laugh_1 ""

# game/script/13.rpy:627
translate german s13_03_71871104:

    # avt "\"How isn't it? We take care of our temples for the gods, clean them, repair them, give sacrifices to them.\" "
    # avt "\"I've been thinking about you even before I could see you. But ever since I knew you were there...\" "
    avt ""

# game/script/13.rpy:629
translate german s13_03_b67d30a7:

    # avt "\"It feels like all I've cared about is myself. I've never really seriously considered what you wanted or needed.\" "
    avt ""

# game/script/13.rpy:631
translate german s13_03_eef6d077:

    # tig normal "\"...You're so mature Liao.\" "
    tig normal ""

# game/script/13.rpy:633
translate german s13_03_1a2d9215:

    # b "Tiger pats my head. "
    b ""

# game/script/13.rpy:635
translate german s13_03_9db85034:

    # avt "\"H-{w=0.2}Hey, you don't need to treat me like a kid.\" " with hshake
    avt "" with hshake

# game/script/13.rpy:637
translate german s13_03_dc9295aa:

    # tig laugh_1 "\"But you're all children to us. Not only you, but your father, and even your grandfather as well. Human life is short after all.\" "
    tig laugh_1 ""

# game/script/13.rpy:644
translate german s13_03_4e33938a:

    # avt "\"Didn't you only come from statue a year ago?\" "
    avt ""

# game/script/13.rpy:646
translate german s13_03_abb7691a:

    # tig laugh_1 "\"Well, it has been at least ten years since my statue was carved.\" "
    tig laugh_1 ""

# game/script/13.rpy:648
translate german s13_03_962cad5e:

    # avt "\"Ten years! You're younger than me!\" "
    avt ""

# game/script/13.rpy:650
translate german s13_03_cf78dc61:

    # tig laugh_1 "\"It makes no difference. It still remains that everyone from the Lin family are the children of the gods.\" "
    tig laugh_1 ""

# game/script/13.rpy:652
translate german s13_03_ebf5be5b:

    # avt "\"Whatever... that still doesn't mean you {i}need{/i} \ to treat me like I'm your kid...\" "
    avt ""

# game/script/13.rpy:659
translate german s13_03_f7f6f5a8:

    # avt "\"It's weird! I'm in college!\" "
    avt ""

# game/script/13.rpy:665
translate german s13_03_2e745039:

    # tig laugh_1 "\"Alright— Alright— I'll treat you like an adult from now on, how does that sound?\" "
    tig laugh_1 ""

# game/script/13.rpy:667
translate german s13_03_03e5f1b1:

    # avt "\"What does that mean?\" "
    avt ""

# game/script/13.rpy:669
translate german s13_03_3ce207a6:

    # tig laugh_2 "\"Oh, you see... {w=0.5}this month's offerings are on you. Does that please you?\" "
    tig laugh_2 ""

# game/script/13.rpy:672
translate german s13_03_6e131847:

    # avt "\"You just want me to buy stuff for you!\" " with vshake
    avt "" with vshake

# game/script/13.rpy:674
translate german s13_03_911284c4:

    # tig laugh_1 "\"Isn't that the responsibility of an adult?\" "
    tig laugh_1 ""

# game/script/13.rpy:676
translate german s13_03_5bc34f61:

    # avt "\"I'm still living on an allowance!\" "
    avt ""

# game/script/13.rpy:678
translate german s13_03_0b885319:

    # tig laugh_1 "\"Well you told me to treat you like an adult.\" "
    tig laugh_1 ""

# game/script/13.rpy:680
translate german s13_03_3e91217b:

    # avt "\"...It sounds more like you want to bully me.\" "
    avt ""

# game/script/13.rpy:685
translate german s13_03_afff4bf6:

    # tig laugh_2 "\"Ha-hahaha—\" " with vshake
    tig laugh_2 "" with vshake

# game/script/13.rpy:687
translate german s13_03_53335191:

    # tig normal "\"But in all seriousness, Liao....\" "
    tig normal ""

# game/script/13.rpy:689
translate german s13_03_0bc21ef0:

    # tig normal "\"Now that they're gone, if you ever feel alone...\" "
    tig normal ""

# game/script/13.rpy:691
translate german s13_03_f8e3a90b:

    # tig gentle "\"I'll do my best to cheer you up, okay?\" "
    tig gentle ""

# game/script/13.rpy:693
translate german s13_03_9888d47c:

    # avt "\"Well, cheer me up and we'll see.\" "
    avt ""

# game/script/13.rpy:695
translate german s13_03_eee1e00a:

    # tig lift "\"What? You don't believe me?\" "
    tig lift ""

# game/script/13.rpy:697
translate german s13_03_5ce325e4:

    # avt "\"Because despite his age, my guardian tiger's a grumpy old man.\" "
    avt ""

# game/script/13.rpy:699
translate german s13_03_634187bb:

    # tig laugh_1 "\"I'll still take good care of you.\" "
    tig laugh_1 ""

# game/script/13.rpy:701
translate german s13_03_cdb759eb:

    # avt "\"...Mm-hmm, thanks.\" "
    avt ""

# game/script/13.rpy:703
translate german s13_03_6ce8c1bc:

    # b "Tiger's strokes this paw through my hair gently. "
    b ""

# game/script/13.rpy:705
translate german s13_03_ea07e4cd:

    # tig normal "\"Now, I believe it's getting late. Time for bed then?\" "
    tig normal ""

# game/script/13.rpy:707
translate german s13_03_2dd7fb8d:

    # avt "\"...Yeah.\" "
    avt ""

# game/script/13.rpy:719
translate german s13_03_d98dc441:

    # b "Tiger's the only one here, so he sleeps with me tonight. "
    b ""

# game/script/13.rpy:721
translate german s13_03_3fdfb68b:

    # b "Even though I slept with Tiger once during the exam break it's not exactly something you get used to quickly... "
    b ""

# game/script/13.rpy:724
translate german s13_03_4e6ba42b:

    # b "Compared to Leopard, Tiger takes up heaps more space than what I'm used to. It takes a while to get comfortable, but in the end I lie down being hugged from behind like last time. "
    b ""

# game/script/13.rpy:726
translate german s13_03_15de9132:

    # b "I know I shouldn't let my imagination go wild but lying in his arms like this... I can't stop my heart from racing... "
    b ""

# game/script/13.rpy:728
translate german s13_03_ed612703:

    # b "I try to distract myself by thinking about other things, but my efforts are just rewarded by thinking over everything that happened today. "
    # b "Tiger notices my distress, stroking his paw through my hair gently. "
    b ""

# game/script/13.rpy:730
translate german s13_03_de4bfca0:

    # b "Between his tender pets and his warm embrace, I find it easy to calm down. Held tight against his body, it makes me feel safe. "
    b ""

# game/script/13.rpy:732
translate german s13_03_90613caa:

    # b "It feels really nice... "
    b ""

# game/script/13.rpy:742
translate german s13_03_451b6df3:

    # b "After turning off the light, Tiger and I lie in bed together. "
    b ""

# game/script/13.rpy:744
translate german s13_03_71d33f8e:

    # b "Without Senpai around since the start of exam break, Tiger's been sleeping with me. "
    b ""

# game/script/13.rpy:746
translate german s13_03_2c6dd83c:

    # b "I haven't really got used to sleeping with him though. It's not bad, but I kinda wish Senpai was here... "
    b ""

# game/script/13.rpy:749
translate german s13_03_a458c61f:

    # b "We sleep together in the same position as usual, his arms embrace me from behind with his hands resting against my chest. "
    b ""

# game/script/13.rpy:751
translate german s13_03_b55cfdfe:

    # b "I nuzzle his hand, feeling the warmth of his soft fur brush up against my face. "
    b ""

# game/script/13.rpy:753
translate german s13_03_1e883066:

    # b "I close my eyes to the rhythm of each of his deep breaths. "
    b ""

# game/script/13.rpy:755
translate german s13_03_ff3b3caf:

    # b "And gradually, fall asleep in the reassuring warmth of his body around me. "
    b ""

# game/script/13.rpy:764
translate german s13_03_d98dc441_1:

    # b "Tiger's the only one here, so he sleeps with me tonight. "
    b ""

# game/script/13.rpy:766
translate german s13_03_d1c96b0a:

    # b "It's my first time sleeping with Tiger. I'm a little bit nervous, after all... "
    b ""

# game/script/13.rpy:768
translate german s13_03_f78b4fe2:

    # b "He's massive! He basically takes up the whole single bed. There's nowhere left for me to sleep... "
    b ""

# game/script/13.rpy:771
translate german s13_03_986603a9:

    # b "We adjust again and again, but no matter what I always find my self in his arms. I'm a bit shy but... "
    b ""

# game/script/13.rpy:773
translate german s13_03_d390fd0d:

    # b "Tiger's arms embrace me from behind, as if he were hugging me tight, protecting me from danger. "
    b ""

# game/script/13.rpy:775
translate german s13_03_90613caa_1:

    # b "It feels really nice... "
    b ""

# game/script/13.rpy:784
translate german s13_03_d9a5e7a6:

    # b "After turning off the light, Tiger and I lie in bed together. "
    b ""

# game/script/13.rpy:787
translate german s13_03_2dd5f3a7:

    # b "We sleep together in the same position as usual, his arms embrace me from behind with his hands resting against my chest. "
    b ""

# game/script/13.rpy:789
translate german s13_03_b55cfdfe_1:

    # b "I nuzzle his hand, feeling the warmth of his soft fur brush up against my face. "
    b ""

# game/script/13.rpy:791
translate german s13_03_1e883066_1:

    # b "I close my eyes to the rhythm of each of his deep breaths. "
    b ""

# game/script/13.rpy:793
translate german s13_03_ff3b3caf_1:

    # b "And gradually, fall asleep in the reassuring warmth of his body around me. "
    b ""

